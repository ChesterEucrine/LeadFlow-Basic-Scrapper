...
import urllib.parse
import re
from CampaignHelper import *

#x = urllib.request.urlopen("https://app.leadflow360.net/login")
#print(x.read())

"""url = "https://app.leadflow360.net/login"
values = {}
...
"""
try:
    x = urllib.request.urlopen("https://www.google.com?q=hope")
    print(x.read())
...
because Chester connected party did not properly respond after a period of time, or est
...
username = "jacksonmeya@gmail.com"
password = "4EggFafp0Z92"
enc_pass = base64.b64decode(password)
...
success = True
try:
    login = ses.post(url, headers=headers1, data=values)
    print("Successfully logged into LeadFlow")
except Exception as e:
    print("Failed to Login to Leadflow")
    success = False
    print(str(e))
...
"""if success:
    try:
        google = ses.get("https://www.googletagmanager.com/gtag/js?id=UA-158989952-1")
        dashboard = ses.get(url2, headers=headers2)
        #webdata.append(dashboard.json())
        # Also check status from the returned string
    except Exception as e:
...
if success:
    try:
        apimin = ses.get(appminurl, headers=appminheaders)
        api = ses.post(url3, headers=headers3, data={"userCampaigns": "true"})
        apijsondata = api.json()
        #print(apijsondata)
        webdata.append(api.json())
        # Also check status from the returned string
    except Exception as e:
        print("Unable to retrieve compaigns")
        print(str(e))
...
#camp = getCompaignData("21881", ses)



path = "./campaigns_data.txt"
#camp_data = "./example.txt"

if success:
    #writeContentTo(path, webdata)
    #writeContentTo(camp_data, [camp])
    #printFrom(camp_data)

    info = getJSONFromFile(path)
    userCampaigns = info["userCampaigns"]
    campaignIDs = getOTCCampaignIDs(userCampaigns)
